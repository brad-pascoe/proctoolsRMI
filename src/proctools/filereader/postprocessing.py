...
def load_planar_average_tecdata(folder: str | Path,time):
    planar_average_dir = Path(folder)/"PlanarAverage"
    planar_average_files = [f for f in planar_average_dir.iterdir() if 'PlanarAverage.tec.dat' in f.name]
    for file in planar_average_files:
        file_time = float(re.findall('(.+).PlanarAverage.tec.dat',file.name)[0])
        if (file_time != time):
            continue
        planar_average_data = load_tecplot(file,True,True)
        return planar_average_data
    raise FileNotFoundError


def load_results_cross(folder: str | Path) -> pd.DataFrame:
    results_cross_filename = "results_cross.dat"
    results_cross_cols = ["Time","XCross","TransverseL","TransverseLAlt"]
    results_cross_file = Path(folder)/results_cross_filename
...
def load_mixed_mass(folder: str | Path) -> pd.DataFrame:
    mixed_mass_filename = "results_mixed_mass.dat"
    mixed_mass_cols = ["Time", "MixedMass","Psi"]
    mixed_mass_file = Path(folder)/mixed_mass_filename
...
def load_spectrum_data(file: str | Path) -> pd.DataFrame:
...
def load_decomposed_spectrum_data(file: str | Path) -> pd.DataFrame:
...
def load_bubblespike_data(folder: str | Path, time2tau: float = 1, wavelength: float = 1) -> pd.DataFrame:
...
def load_planar_average_data(folder: str | Path,time):
    planar_average_cols = ["X","FavreU","FavreV","FavreW","Pressure","Y1","f1","Density","TKE","TKE2","MeanU","MeanV","MeanW","f1f2"]
    planar_average_dir = Path(folder)/"PlanarAverage"
    planar_average_files = [f for f in planar_average_dir.iterdir() if '.PlanarAverage.dat' in f.name]
    for file in planar_average_files:
        file_time = float(re.findall('.+(?=.PlanarAverage.dat)',file.name)[0])
        if (file_time != time):
            continue
        planar_average_data = pd.read_fwf(file,header=None,names=planar_average_cols)
        return planar_average_data
    raise FileNotFoundError


def load_planar_average_dataV2(folder: str | Path,time):
    ...
    planar_average_dir = Path(folder)/planar_average_folder
    planar_average_files = list(planar_average_dir.iterdir())
    for file in planar_average_files:
        file_time = float(re.findall('.+(?=.PlanarAverage2.dat)',file.name)[0])
        if (file_time != time):
            continue
        planar_average_data = pd.read_fwf(file,header=None,names=planar_average_cols)
        return planar_average_data
...
def load_tke_data(folder: str | Path, time2tau: float = 1, tke_scaling: float = 1) -> pd.DataFrame:
...
def load_favre_stress_data(case_folder: str | Path, time: float) -> pd.DataFrame:
    rs_file = Path(case_folder)/"ReynoldsStress"/f"{time:.8f}_Favre.dat"
